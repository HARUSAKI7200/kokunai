...
class SlashedRectangle extends DrawingElement {
  ...
  @override
  SlashedRectangle clone() =>
      SlashedRectangle(id: id, start: start, end: end, paint: Paint.from(paint));
  ...
  factory SlashedRectangle.fromJson(Map<String, dynamic> json) => SlashedRectangle(
        id: json['id'],
        start: Offset(
            json['start']['dx'].toDouble(), json['start']['dy'].toDouble()),
        end: Offset(json['end']['dx'].toDouble(), json['end']['dy'].toDouble()),
...
class DrawingText extends DrawingElement {
  ...
  void draw(Canvas canvas, Size size) {
    final textSpan = TextSpan(
        text: text,
        style: TextStyle(
            color: paint.color, fontSize: 30, fontWeight: FontWeight.bold));
    final textPainter =
